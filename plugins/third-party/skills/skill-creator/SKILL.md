...
> **Kilo note (local addition, not upstream):** this section and the whole
> with-skill/without-skill benchmark below it (`run_eval.py`, `run_loop.py`,
> `improve_description.py`, and the `grader`/`comparator`/`analyzer`
> subagents) shell out to the `claude` CLI specifically (`claude -p`) —
> there is no Kilo-native equivalent path. When invoked from Kilo alone,
> skip straight to manual review; these scripts only work when the `claude`
> binary is actually reachable (e.g. running inside, or alongside, an
