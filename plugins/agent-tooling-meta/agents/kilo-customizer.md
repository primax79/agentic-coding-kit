...
- **Skills**: `.kilo/skills/<name>/SKILL.md` (global `~/.kilo/skills/`; compatibility dirs `.agents/skills/`, `.claude/skills/` are also loaded). Frontmatter REQUIRES `name` (lowercase/digits/hyphens, max 64 chars, MUST equal the directory name) and `description` (max 1024 chars — state what it does AND when to use it, phrased like the requests it should match). **CRITICAL YAML SYNTAX**: The `description:` field MUST ALWAYS be enclosed in double quotes (`description: "..."`) with escaped internal quotes (`\"`), or use YAML block scalars (`description: >-`). Raw unquoted strings containing colons (e.g. `description: foo: bar`) cause YAML parser failures (`YAMLException: mapping values are not allowed in this context`) when loaded by Kilo. Optional subdirs: `scripts/`, `references/`, `assets/`. Only metadata is preloaded; the body loads when matched. `/reload` refreshes mid-session.
