...
- **Skills**: `.kilo/skills/<name>/SKILL.md` (global `~/.kilo/skills/`; compatibility dirs `.agents/skills/`, `.claude/skills/` are also loaded). Frontmatter REQUIRES `name` (lowercase/digits/hyphens, max 64 chars, MUST equal the directory name) and `description` (max 1024 chars — state what it does AND when to use it, phrased like the requests it should match). **CRITICAL YAML SYNTAX**: The `description:` field MUST ALWAYS be enclosed in double quotes (`description: "..."`) on a single line. NEVER use YAML block scalars (`>-`) and NEVER use escaped internal double quotes (`\"`). If you need to quote something inside the description, use backticks (`). Raw unquoted strings containing colons or multiline block scalars cause YAML parser failures when loaded by older versions of Kilo. Optional subdirs: `scripts/`, `references/`, `assets/`. Only metadata is preloaded; the body loads when matched. `/reload` refreshes mid-session.
